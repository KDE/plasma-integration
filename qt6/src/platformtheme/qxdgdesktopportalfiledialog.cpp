...
void QXdgDesktopPortalFileDialog::openPortal(Qt::WindowFlags windowFlags, Qt::WindowModality windowModality, QWindow *parent)
{
    ...
    if (d->saveFile && !d->selectedFiles.isEmpty()) {
        ...
        if (selectedFileInfo.exists())
            options.insert("current_file"_L1, QFile::encodeName(d->selectedFiles.constFirst()).append('\0'));
    ...
    if (!d->mimeTypesFilters.isEmpty()) {
        for (const QString &mimeTypefilter : std::as_const(d->mimeTypesFilters)) {
        ...
        for (const QString &nameFilter : std::as_const(d->nameFilters)) {
            // Do parsing:
            ...
            if (match.hasMatch()) {
                ...
                const QStringList filterStrings = match.captured(2).split(u' ', Qt::SkipEmptyParts);
    ...
    auto unixServices = dynamic_cast<QGenericUnixServices *>(QGuiApplicationPrivate::platformIntegration()->services());
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this, d, windowFlags, windowModality, parent](QDBusPendingCallWatcher *watcher) {
        ...
        if (d->failedToOpen) {
            ...
            QDBusConnection::sessionBus()
                .connect(nullptr, reply.value().path(), "org.freedesktop.portal.Request"_L1, "Response"_L1, this, SLOT(gotResponse(uint, QVariantMap)));
        }
        watcher->deleteLater();
    });
}
