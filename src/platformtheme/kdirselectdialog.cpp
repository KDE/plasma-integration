...
/*!
    \internal
    \a url is a QUrl generated by calling QUrl::fromUserInput on \a text
    This method checks if text has trailing spaces, and if so, checks if
    \a url + the trailing spaces is a file that exists. If so, \a url is
    replaced with an URL pointing to it.
 */
static void handleFileWithSpaces(const QString &text, QUrl *url)
{
    // Doing a stat in a synchronous function is probably to expensive
    if (text.isEmpty() || !(text.cend()-1)->isSpace() || !url->isLocalFile())
        return;
    // remove the scheme because QFileInfo wants the path without it
    QString urlString = url->toString(QUrl::StripTrailingSlash | QUrl::RemoveScheme);
    auto firstNonSpaceIt = std::find_if(text.rbegin(), text.rend(),[](QChar val) {
        return !val.isSpace();
    });
    // append the trailing white space
    urlString += text.right(std::distance(text.rbegin(), firstNonSpaceIt));
    if (QFileInfo::exists(urlString)) { // does the file with spaces actually exist?
        *url = QUrl("file:" + urlString); // QUrl constructor wants the scheme
    }
    qDebug() << "final" << *url << "from" << urlString;
}
...
void KDirSelectDialog::Private::slotUrlActivated(const QString &text)
{
    ...
    // fromUserInput might have stripped spaces from a filename containing them
    // this doesn't matter for the isLocalFile check above, but we need to check
    // for it before actually calling setCurrentUrl
    handleFileWithSpaces(text, &url);
...
QUrl KDirSelectDialog::url() const
{
    ...
    handleFileWithSpaces(d->m_urlCombo->currentText(), &comboUrl);
